...
export const ApplicantSchema = z.object({
  ...
  cgpa: z.number().min(0, 'CGPA must be at least 0').max(10, 'CGPA cannot exceed 10').nullable().optional(),
...
export const EvaluationSchema = z.object({
  ...
  ratingTechnicalSkills: z.number().int().min(1).max(5).nullable().optional(),
  ratingProblemSolving: z.number().int().min(1).max(5).nullable().optional(),
  ratingTeamFit: z.number().int().min(1).max(5).nullable().optional(),
  ratingConfidence: z.number().int().min(1).max(5).nullable().optional(),
  ratingGrowthMindset: z.number().int().min(1).max(5).nullable().optional(),
  ratingLeadership: z.number().int().min(1).max(5).nullable().optional(),
  suitableForTechnical: z.enum(['YES', 'NO', 'UNDECIDED']).nullable().optional(),
  suitableForNonTechnical: z.enum(['YES', 'NO', 'UNDECIDED']).nullable().optional(),
